...
void loop()
{
  ...
  if (ttext > 0)
  {
    char *text0 = "Greetings to YouTube!";
    char text[32];
    int i = 0;
    for (; i < ttext && i < 32; i++)
    {
    ...
    graphics.setTextColor(graphics.rgb(50, 60, 200));
    graphics.print("Temperature: ");
    graphics.print(tmp);
    graphics.print(" C Humidity: ");
    graphics.print(h);
    graphics.print("/100");
  }
  ...
  if (ttext > 0)
  {
    ...
    text[i] = 0;
    graphics.setCursor(160 - 4 * i, 220);
    graphics.setTextColor(graphics.rgb(68, 205, 127));
    graphics.print(text);
